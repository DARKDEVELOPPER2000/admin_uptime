...
function wireGlobalUI(){
  ...
  if (logout){
    logout.addEventListener('click', () => {
      toast('Session', 'Déconnexion (mock).', 'ok');
      closeSidebar();
      location.hash = '#/login';
    });
  }
  ...
  $('#btn-quick-job')?.addEventListener('click', () => { closeSidebar(); go('#/job-create'); });
  $('#btn-quick-account')?.addEventListener('click', () => { closeSidebar(); go('#/accounts-create'); });

  // Global search
  const input = $('#global-search');
  input?.addEventListener('keydown', (e) => {
    if (e.key === 'Enter'){
      toast('Recherche', `Recherche: "${input.value}" (mock)`, 'ok');
    }
  });
...
main().catch(err => {
  console.error(err);
  toast('Erreur', 'Impossible de démarrer l’application.', 'danger');
});
